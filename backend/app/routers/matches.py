...
def _existing_identity_for(db: Session, material_id: str) -> str | None:
    m = db.query(models.MaterialMatch).filter(
        ...
        (
            (models.MaterialMatch.material_a == material_id)
            | (models.MaterialMatch.material_b == material_id)
        ),
    ).first()
...
def _match_out(db: Session, match: models.MaterialMatch) -> schemas.MatchOut:
    return schemas.MatchOut(
        id=match.id,
        material_a=match.material_a,
        material_b=match.material_b,
...
def list_matches(
    ...
    matches = (
        q.order_by(models.MaterialMatch.final_confidence.desc())
        .all()
    )
...
@router.post("/rebuild")
def rebuild_pending_matches(db: Session = Depends(get_db)):
    """
    Rebuild only unresolved/pending candidates.

    Accepted and rejected matches are preserved.
    Existing material and identity records are never modified.
    """

    pending_deleted = (
        db.query(models.MaterialMatch)
        .filter(models.MaterialMatch.status == "pending")
...
def resolve_match(
    ...
    if match.status != "pending":
        raise HTTPException(
            status_code=400,
            detail=f"match already {match.status}",
        )
    ...
    if body.action == "reject":
        ...
        if match.conflict_reason:
            raise HTTPException(
                status_code=400,
                detail=f"hard conflict prevents match: {match.conflict_reason}",
            )

        identity_id = (
            _existing_identity_for(db, match.material_a)
            or _existing_identity_for(db, match.material_b)
        )
        ...
        if not identity_id:
            ...
            name = (
                body.canonical_name
                or mat_a.normalized_description
                or mat_a.description
            )

            identity = models.MaterialIdentity(
                canonical_name=name,
